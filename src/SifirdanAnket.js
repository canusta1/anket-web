...
function SifirdanAnket() {
    ...
    const handleGorselYukle = (id, file) => {
        ...
        reader.onload = (e) => {
            setSorular(sorular.map(s => s.id === id ? { ...s, gorselUrl: e.target.result } : s));
        };
        reader.readAsDataURL(file);
    };
    
    // Görsel silme handler
    const handleGorselSil = (id) => {
        setSorular(sorular.map(s => s.id === id ? { ...s, gorselUrl: null } : s));
    };
                                                        ...
                                                        <div className="q-settings-row">
                                                            ...
                                                            <label className={`q-toggle-btn q-image-btn ${activeQ.gorselUrl ? 'has-image' : ''}`}>
                                                                <input
                                                                    ...
                                                                    accept="image/*"
                                                                    style={{ display: 'none' }}
                                                                    onChange={(e) => handleGorselYukle(activeQ.id, e.target.files[0])}
                                                                />
                                                        ...
                                                        <div className="q-preview-section">
                                                            ...
                                                            {activeQ.gorselUrl && (
                                                                <div className="q-image-preview">
                                                                    ...
                                                                    <button 
                                                                        className="q-image-remove"
                                                                        onClick={() => handleGorselSil(activeQ.id)}
                                                                        type="button"
                                                                    >
